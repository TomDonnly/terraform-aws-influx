...
function mock_instance_json {
  ...
  local influxdb_ip
  influxdb_ip=$(getent hosts "$container_name" | awk '{ print $1 }')
  assert_not_empty_or_null "$influxdb_ip" "hostname for container $container_name"
  local influxdb_hostname="$(hostname)"

  cat << EOF
{
  "LaunchTime": "$launch_time",
  "InstanceId": "$instance_id",
  "PublicIpAddress": "$influxdb_ip",
  "PrivateIpAddress": "$influxdb_ip",
  "PrivateDnsName": "$influxdb_hostname",
  "PublicDnsName": "$influxdb_hostname",
  "Tags": [
    {
      "Value": "$asg_name",
      "Key": "Name"
