...
variable "ami_id" {
  description = "The ID of the AMI to run in this server."
  type        = string
}

variable "instance_type" {
  description = "The type of EC2 Instances to run for each node in the server (e.g. t2.micro)."
  type        = string
}

variable "vpc_id" {
  description = "The ID of the VPC in which to deploy the Kapacitor server"
  type        = string
}
...
variable "allowed_ssh_security_group_ids" {
  description = "A list of security group IDs from which the server will allow SSH connections"
  type        = list(string)
  default     = []
}
...
variable "associate_public_ip_address" {
  description = "If set to true, associate a public IP address with the server."
  type        = bool
  default     = false
}
